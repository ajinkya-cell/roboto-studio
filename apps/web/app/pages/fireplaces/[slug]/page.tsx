...
async function getFireplace(slug: string) {
  const query = `*[_type == "fireplaces" && slug.current == $slug][0]{
    title,
    category,
    type,
...
export default async function FireplaceDetailPage({ params }: PageProps) {
  const { slug } = await params;
  if (!slug) notFound();

  const data = await getFireplace(slug);
  if (!data) notFound();
