...
async function getAllFireplaces(): Promise<FireplaceItem[]> {
  const query = `*[_type == "fireplaces"] | order(_createdAt desc){
    _id,
    title,
    slug,
    category,
...
export default async function FireplaceListingPage() {
  const items = await getAllFireplaces();
      ...
      <div className="mx-auto h-screen max-w-7xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
          {items.map((item) => (
            <Link
              key={item._id}
              href={`/pages/fireplaces/${item.slug.current}`}
              className="group"
            >
