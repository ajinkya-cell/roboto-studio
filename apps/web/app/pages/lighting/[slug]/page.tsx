...
async function getLighting(slug: string) {
  const query = `*[_type == "lighting" && slug.current == $slug][0]{
    title,
    category,
    type,
...
export default async function LightingDetailPage({ params }: PageProps) {
  const { slug } = await params;
  if (!slug) notFound();

  const data = await getLighting(slug);
  if (!data) notFound();
