...
async function getAllLighting(): Promise<LightingItem[]> {
  const query = `*[_type == "lighting"] | order(_createdAt asc){
    _id,
    title,
    slug,
    category,
...
export default async function LightingListingPage() {
  const items = await getAllLighting();
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
          {items.map((item) => (
            <Link
              key={item._id}
              href={`/pages/lighting/${item.slug.current}`}
              className="group"
            >
